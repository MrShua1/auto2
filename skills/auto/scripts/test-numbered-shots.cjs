...
test('undefined subject in action fails', () => { const f = fixture(); assert.throws(() => f.run(f.prompt.replace('主体1轻提', '主体99轻提')), /undefined\/nonvisual subject/); });
test('spoken fictional subject label does not create an asset reference', () => { const f = fixture(); assert.equal(f.run(f.prompt.replace('台词/O.S./OS：无。', '台词/O.S./OS：主体1说：“主体99是文件里的编号。”')).timedRanges, 2); });
