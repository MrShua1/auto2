---
name: auto2
description: "Auto2 专为 Seedance 2.0 (SD 2.0) 等 10~15 秒短视频模型量身定制的工业级影视分集与预视频工程。单分段（SEG）时长刚性控制在大于10秒、小于等于15秒（11~15s），单段 2~3 镜头，台词容量 15~35 字。支持 /auto batch20（推进1-20集）等批次指令。默认 20 并发分析架构。严格执行四大支柱、真实大模型逐幕物理动作转译与严格0视频生成。"
---

# Auto2 影视分集工程规范（Auto2 Screenplay-to-Prevideo Production · SD 2.0 专用版）

本技能专门用于短剧剧本到预视频交付工程（Pre-video Engineering Package）的专业分集与分镜制作，针对 **Seedance 2.0 (SD 2.0)** 等 10~15 秒短视频大模型进行专用视听语法与时长刚性适配。
...
### 15. 【台词剧情容量、动作分级与分段结束边界铁律（Rule 0.19，7.2/7.3/7.4规范 · SD 2.0 10~15秒专用）】
- **【台词与剧情容量刚性门禁（Dialogue & Plot Capacity Rule，7.2规范）】**：
  * **单段时长硬锁（10~15秒）**：Auto2 专为 Seedance 2.0 (SD 2.0) 等短视频模型设计，每一分段（SEG）生成时长严格锁定在 **大于 10 秒，且小于等于 15 秒（即 11 ~ 15 秒）**；凡 ≤ 10 秒或 > 15 秒直接触发拦截一票否决；
  * **单段容量基准**：每 10~15 秒 Prompt 台词容量建议为 **15—35 字**；单段镜头数建议为 **2—3 个镜头**；单镜头台词建议 6—14 字；戏剧强爆点镜头 3—8 字；
  * **短句承载**：3—8 字短句若非重大反转、身份揭露、关键证据或强爆点，必须继续承载后续原文剧情，严禁无意义切片；
  * **中长句整合**：8—18 字镜头优先整合后续台词、有效动作、反应或结果；15—35 字结合人物语速与动作复杂度综合界定分段边界；
  * **超长对白拆分红线**：原文台词超过 35 字时，**必须原样拆分到多个镜头或独立 Prompt（SEG），严禁删改对白、严禁颠倒顺序、严禁异常倍速快进**；
  * **严禁虚构台词**：剧本台词不足 15 字时绝对严禁擅自添油加醋补写台词；“70%动作+30%台词”仅作为视听节奏参考。
...
- **【镜头有效性与 Prompt 结束分段边界铁律（Shot Validity & Segment Boundary Gate，7.4规范）】**：
  ...
  * **结束 Prompt 的唯一合法条件**：仅在 **11~15 秒容量已饱和**（大于10秒，≤15秒）、完整剧情节点结束、继续承载会导致超时/破坏正常说话语速（语速超过 4.5 字/秒）、场景/时间切换，或关键爆点与悬念钩子需要强化时，方可结束当前 Prompt 切入下一 SEG；严禁为了强凑段数而硬塞超载台词，严禁生成小于等于 10 秒或大于 15 秒的非法片段；
