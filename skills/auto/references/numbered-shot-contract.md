...
## Physical States Only & Lineart Mapping (Rule 0.24)

Opening section (【站位与起始状态】) represents the physical spatial anchor of the segment and must be 100% identical in physical composition, screen coordinates, and body/gaze directions to Shot 1 (【镜头1】).
- **Mandatory Lineart Mapping**: The opening section MUST declare lineart-subject mapping at its very start:
  [线稿图对应关系：主体1=某角色（站位/朝向），主体2=某角色（站位/朝向）] (or [镜头1线稿图对应关系：...]).
- **Physical Invariance with Shot 1**: All character coordinates, facing vectors, depth layers, and relative distances must 100% match Shot 1's lineart sketch and 位置承接：.
- **Zero Wardrobe Declaration**: Pure physical spatial staging only (Rule 0.21); zero clothing words.
