...
0.24 【分镜图与站位线稿三阶解耦必备流程铁律 · 彻底粉碎正脸肖像偏见（Mandatory Storyboard Keyframe Image & Spatial Lineart Decoupling Gate）】：
   ...
   - **【空间骨架层：极简黑白站位线稿硬锁三维空间（Pure Black-and-White Spatial Lineart Rule）】**：
     * 每一个出镜镜头**必须生成/配备 1 张纯净黑白站位线稿（`SEG00x_镜头y_站位线稿`）**；
     * 绘制规范：极简白底黑线（火柴人/简笔轮廓线），**仅体现画面横向坐标（画左/画中/画右）、纵深高低差、景别、身体朝向（侧身对峙、正向微侧）、肢体动态重心与视线交汇夹角**；
     * **故意不画五官、故意不画衣服细节**：纯粹作为三维几何空间骨架（Spatial Skeleton），彻底消除文生图/视频模型默认的“肖像照先验偏见（Portrait Prior）”，从物理上切断“出场人物本能看镜头/看屏幕”的致命偏见；
     * 生产加速：支持真实 AI 接口（如 st1 `gpt-image-2.5-sunburst`，1792x1024）启用 **20 并发极速生成线稿**。
