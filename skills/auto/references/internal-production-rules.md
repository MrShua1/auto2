...
0.18 【声画对白绝对人嘴开口与OS画外音严格隔离铁律 · 彻底废除 O.S. 标签污染（Verbatim Lip-Sync Speech, Zero Label Pollution & Strict OS Voiceover Isolation Gate）】：
   - **【最高绝对红线：原著剧本未标注 OS 则必须 100% 物理张嘴开口发音（Zero Unmotivated Voiceover Rule）】**：
     * **原著 OS 独占原则（Script-OS Exclusivity）**：只有原著剧本显式标注 `(OS)`、`（OS）`、`（内心OS）` 的台词，才具备合法资格定性为“内心独白 / 画外音（O.S.）”；
   ...
   - **【彻底清除字段标签中的 O.S./OS 污染词（Label Decontamination Axiom）】**：
     ...
     * 字段标签与类型严格二分：
       ...
       ② **内心独白 / 纯画外音（原著有OS）**：字段标签使用 `台词（画外音）：` 或 `画外音（O.S.）：`，若人物出镜必须显式锁死：
...
0.32 【台词原著 OS 独占与绝对严禁假 OS 及抽象心理词最高铁律 · 彻底杜绝口型与语义穿帮（Rule 0.32，Strict Script-OS Exclusivity & Anti-Abstract Thoughts Gate）】：
...
0.33 【资产绝对保真与严禁移花接木/滥竽充数铁律 · 缺失资产全自动合规生成门禁（Rule 0.33，Strict Asset Fidelity, Anti-Substitution & Autonomous Asset Synthesis Gate）】：
   ...
   - **【缺失资产全自动合规生成与补齐管线（Autonomous Asset Synthesis Pipeline）】**：
     * **主动探测与合规自生（Proactive Detection & Autonomous Generation）**：当解析原著剧本场次与动作行发现核心道具/场景资产缺失时，Auto 调度引擎严禁摆烂报错或胡乱借用，**必须立即启动自主影视级资产图生成引擎**，根据剧本微观物理描述生成标准资产参考图；
     * **Auto 框架道具资产标准（Prop Asset Standards）**：
       ① **画幅与清晰度**：16:9 比例，8K 超清写实摄影质感；
       ② **微观物理细节**：忠实还原原著描述的材质、磨损痕迹、配件与结构（如小破渔网：经年风化尼龙粗绳网孔、纯手工麻线修补补丁痕迹、边缘圆柱形深灰铅坠沉子配重）；
       ③ **中性纯净背景与防手持偏见**：纯净无杂质的木质甲板/中性台面背景，**绝对零人体肢体、零手部入镜（Strictly zero human hands/limbs），彻底根绝道具卡的手持先验偏见**；
     * **多级镜像归档（Multi-Level Mirror Archiving）**：自生资产图必须赋予标准资产编号（如 `PROP_017_SMALL_WORN_NET_v1.png`），同步镜像落盘至：
       - 全局资产库：`资产/道具/`；
       - 分集资产目录：`第XXX集/资产/道具/`；
       - 分段资产目录与素材映射表：各关联 SEG 的 `资产/`、`episode-package-config.json` 与 `素材映射.txt`。
   - **【自生资产透明通报与可追溯交付报告（Mandatory Transparency & Disclosure Gate）】**：
     * 凡发生自主生图补齐缺失资产的情况，Auto 引擎**必须在批次质检验收报告与终端交付输出中，显式向用户提交《新增自生资产备案明细》**，严禁隐瞒偷跑：
       1. 资产全名与资产 ID（如 `PROP_017_SMALL_WORN_NET_v1.png` · 小破渔网）；
       2. 原著剧本出处与缺失原因（如“第003集第3-1场剧本规定黄子名在船头使用小破渔网绑铅坠捕鱼，资产库缺少旧撒网道具”）；
       3. 生成提示词（Prompt）与物理规格参数；
       4. 镜像归档路径与关联生效的 SEG 分段编号。
   - **【资产语义一致性校验门禁（Semantic Asset Conformance Validator Gate）】**：
...
0.34 【台词分段换行排版与分号刚性闭合铁律 · Review 一票否决门禁（Rule 0.34，Strict Dialogue Semicolon Termination & Paragraph Line-Break Gate）】：
   ...
   - **【Review 刚性闸门与一票否决机制（Review Gate & Instant Disqualification）】**：
     * **硬性评审门禁**：在 Auto 所有人工审核、子智能体 Review 审查以及自动化质检（`validate-video-prompts.cjs`）中，**Rule 0.34 设立为刚性评审红线门禁**；
     * **不满足一律不通过（Exit 1）**：凡分镜提示词（Prompt）中存在任何多段台词挤占单行、或任一台词段落末尾缺少分号 `；` / `;` 的情形，**Review 审查必须直接判定为【不通过（FAIL）】，阻断流水线流转与预视频交付，强制要求修复台词排版后方可重新提交！**

0.35 【多分段自然承载、单镜头单一物理空间与零网文浮夸形容词铁律 · Review 一票否决门禁（Rule 0.35，Multi-Segment Hierarchy, Single Physical Space & Zero Web-Novel Fluff Gate）】：
   - **【最高绝对红线：多场戏多分段充分承载，严禁整集强制压缩为过少分段（Strict Multi-Segment Natural Hierarchy Rule）】**：
     * 短剧单集（1~2分钟）剧本包含多个物理场景变换与数十条动作/对白，必须根据数学声学前置测算引擎（`acoustic-profile.json`）锁定的目标分段数科学拆解为独立分段（单 SEG 严格保持 15~28 秒，单镜语速严禁超过 4.0 字/秒）；
     * 绝对严禁为了追求“少几个分段”而把包含多场戏、多物理地点的全集粗暴生硬压缩为 1~2 个 SEG，亦严禁机械无脑套用 4 段！违背声学物理规律必然导致单镜头时空跨越、台词疯狂超载（如9秒塞145字）与严重穿帮。
...
0.36 【动作表演绝对禁入台词与引号铁律 · 剧本原著台词唯一驻留门禁（Rule 0.36，Zero Dialogue in Action Gate）】：
...
0.37 【声学物理动态分段与严禁机械4段论一刀切铁律 · 三层物理化工业流水线门禁（Rule 0.37，Acoustic Dynamic Partitioning & Zero Mechanical 4-SEG Gate）】：
   - **【最高绝对红线：严禁凭主观经验设定死板分段数量（Zero Fixed Segment Rule）】**：
     * 严禁在派发词、代码模板或大模型提示词中使用“通常 3~4 个 SEG / 标准 3~4 个分段”等任何诱导性固定数字！
     * 极度长或极度短的剧集若被机械一刀切硬塞入 4 个 SEG，必然诱发“单镜 9 秒狂塞 145 字台词、语速飙至 13.4 字/秒、动作被完全吞噬”等违背基本物理声学规律的严重生产事故；
     * 每一集的 SEG 数量必须由真实声学字数与动作物理耗时动态计算得出，严禁凭空拍脑袋！
   - **【三层物理化工业流水线架构（Three-Layer Physicalized Industrial Pipeline）】**：
     * **Layer 1（数学声学前置测算 · Acoustic Pre-computation Engine）**：
       - 在大模型动笔前，运行 `skills/auto/scripts/acoustic-precompute.py`，根据剧本纯对白字数（按自然语速 3.6 字/秒测算）与 A/B/C 级动作物理耗时（A级3.0s、B级1.5s、C级0s）及场景切换缓冲（1.5s），正向测算出剧本真实物理总时长；
       - 严格根据单 SEG 15~28 秒物理安全容量区间，动态求解黄金 SEG 目标数量 $N$（及合规区间 $[N_{\min}, N_{\max}]$），生成每集根目录下的 `acoustic-profile.json`；
       - 将正向测算出的分段目标作为不可逾越的刚性契约注入 Worker 派发文本中。
     * **Layer 2（LLM 视听节点确定性物理切片 · Deterministic Audio-Visual Node Slicing）**：
       - 运行 `skills/auto/scripts/deterministic-slice.py`，基于动态规划（DP）全局最优划分算法，将原著剧本平滑切分为 $N$ 个片段；
       - 切片边界优先精准吸附在场景切换标头（`^\d+-\d+`）与 `△`/`▲` 舞台动作行，绝对严禁在人物对白中途截断；
       - 确保所有分段的 `script-verbatim.txt` 拼接后 100% 逐字覆盖权威剧本底本，零漏句、零加字。
     * **Layer 3（刚性代码门禁熔断 · Strict Dialogue Capacity CI Gate）**：
       - 运行 `skills/auto/scripts/validate-dialogue-capacity.py`，对生成的 `prompt.txt` 与分镜执行严格的声学物理门禁检测：
         ① **单镜语速硬熔断**：单镜头语速绝对严禁超过 **4.2 字/秒**（超过直接报 `[HARD FAIL]` 熔断阻断交付）；
         ② **单镜字数硬熔断**：单镜头台词纯字数绝对严禁超过 **35 字**；
         ③ **单 SEG 台词硬熔断**：单 SEG 累计台词绝对严禁超过 **55 字**；
         ④ **分段下限硬熔断**：实际分段数量若低于声学最低底限 $N_{\min}$，一票否决打回重做！
