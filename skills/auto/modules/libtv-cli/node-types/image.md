...
**图片生成节点**：既可做**文生图**（无上游参考图），也可做**图生图 / 参考图**（上游若干张 `image` 资源节点）。画布上的 `image` 节点同时承担「展示产物 / 承载 URL」这一身份，因此**生成参数**（`-s`）与**节点自身属性**（`-u`）都很常用。常见相邻节点：上游 `image` / `text`；下游 `image` / `video` / `video-clip`。
...
| **`scriptRowHiddenUuid`**     | `string`   | 与结构化文本行的关联 UUID。                                |
