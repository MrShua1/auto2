...
metadata:
  version: "2.0.0"
  author: "wangzhang-wu"
  local_patch: "user-authored-shot-lock/1.0"
---
...
### User-authored shot lock

An explicit per-shot direction from the user is source authority, not a suggestion.
When the user names a shot number or order together with any of subject, shot size,
viewpoint, angle, screen direction, action instant, or cut position:

1. Mark every supplied field `LOCKED` and preserve the sequence order verbatim.
2. Fill only unspecified craft fields such as lens, camera height, duration, light,
   depth, continuity notes, and generation risk.
3. Never merge, delete, reorder, substitute, widen, tighten, or re-angle a locked
   shot in the name of coverage, style, model reliability, or "more cinematic"
   results.
4. Coverage and 180-degree/30-degree advice may expose a conflict, but may not
   silently repair it. If two locked instructions are physically incompatible,
   name the exact conflict and ask at most one decisive question.
5. Before delivery or generation, audit each output row and keyframe prompt against
   the user's lock. A mismatch blocks generation.
